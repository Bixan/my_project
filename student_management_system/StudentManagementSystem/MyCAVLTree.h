...
========================================================*/
template<typename T>
bool CAVLTree<T>::Insert(T nStuID, 
	CString szStuName, 
	int nBirthYear, 
	int nBirthMonth, 
	int nSex)
...
/*=======================================================
*	函数功能：根据传入的参数，查找树中存在的所有相同姓名的学生信息
*	参数：
		T nStuID -- 树结点对应的学生ID
		CString szStuName -- 要查询的联系人姓名
*	返回值：
		true -- 数据查找成功
		false -- 结点为空，返回false
*	警告：无
...
typename CAVLTree<T>::PSTUNODE CAVLTree<T>::CopyNewData(PSTUNODE pNode)
{
	if (pNode != nullptr)
	{
		...
		pTempRoot = new STUNODE(pNode->m_nStuID, 
			pNode->m_szStuName, 
			pNode->m_nYear, 
			pNode->m_nMonth, 
			pNode->m_nSex, 
...
/*=======================================================
*	函数功能：调用WirteAVLTreeStuDataToInFile1()方法
		使用递归将树中对应的数据写入到文件
*	参数：无
*	返回值：无
*	警告：无
...
/*=======================================================
*	函数功能：删除源文件，并将备份的文件更改为原来的文件名，用于下次打开使用
*	参数：无
*	返回值：文件打开失败，输入错误信息，退出程序
*	警告：无
*	更新：2020/06/15
